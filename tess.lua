...
-- delay (ms) to wait after a MAGPLANT empty before binding next spot (2-3s recommended)
...
local function BindCurrentMAGSpot()
...
AddHook("OnVariant", "PTHT_MAGEmptyHandler", function(v)
  ...
  if allMagSpotsEmpty() then
    ...
    -- pause planting loops by setting waitingForMagplant = true
    ...
    if useMagSpot and waitingForMagplant then
      if #MAGSpot == 0 then
        ...
        if not found then
          ...
          -- set next index and mark pending bind so handler can map incoming bubble correctly
          currentMAGIndex = found
          pendingMAGBind = found

          -- wait a short delay before binding to avoid immediate/stale talkbubble being misinterpreted
          Log("`2MAG: waiting " .. tostring(mag_switch_delay_ms/1000) .. "s before binding MAG Spot #" .. tostring(found))
  ...
  while true do
    ...
    if reqBindMAG then
      ...
      if MAGSpot[reqBindMAGIndex] then
        currentMAGIndex = reqBindMAGIndex
        MAGSpotEmpty[reqBindMAGIndex] = false
        pendingMAGBind = currentMAGIndex

        -- if user requested a manual bind, also give the small delay to avoid leftover bubble problems
